...
******************************************************************************************************************************************************************************************************/
     if(strcmp(params.bp_name, "bimodal") == 0)                     						// Bimodal
	...
	else if(strcmp(params.bp_name, "hybrid") == 0)											// Hybrid
		{
			...
			if (x>=2)
			{
				if( (outcome=='n' && G_for_hybrid[new_index]>=2) || (outcome=='t' && G_for_hybrid[new_index]<=1) )
				c1++;
				UpdateCounterValues(G_for_hybrid,new_index,outcome);
			}
			else if (x<=1)
			{
				if ((outcome=='n' && B_for_hybrid[index]>=2) || (outcome=='t' && B_for_hybrid[index]<=1))
					c1++;
				UpdateCounterValues(B_for_hybrid,index,outcome);		
			}
