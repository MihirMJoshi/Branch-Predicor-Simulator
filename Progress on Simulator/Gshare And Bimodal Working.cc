...
void Bimodal(unsigned long int B[], unsigned long int index, unsigned long int sets, char outcome, int G[])
{
...
int G_Share(unsigned long int B[], unsigned long int index, unsigned long int sets, char outcome, int G[])
{
     ...
     if(strcmp(params.bp_name, "bimodal") == 0)                     // Bimodal
		{
	 		...
	 		Bimodal(B,index,sets,outcome,G);
	 	}

	else if(strcmp(params.bp_name, "gshare") == 0)           		// Gshare
		{
			index=GetIndex(addr, params.M1,0);
			unsigned long int new_index=G_Share(B,index,sets,outcome,G);
			Bimodal(B,new_index,sets,outcome,G);
		}
