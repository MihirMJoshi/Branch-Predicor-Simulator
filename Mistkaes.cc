...
int GetIndex (unsigned long int addr, unsigned long int size_of_pc, int minus)
{
    ...
    while (n!=0 && q>0)
	 ...
	 if (minus==0)
    	...
    	{
    		sum=sum+pow(2,(31-i-params.N) )*binary[i];
    		 //printf("%d", binary[i]);
    		 //printf("%d : %d : %d\n", i, binary[i], 32-i-(params.N));
		}
     	return sum;
}
...
void G_Share(unsigned long int B[], unsigned long int index, unsigned long int sets, char outcome, int G[])
{
	...
	unsigned long int m_minus_n_for_g_share = GetIndex(index,params.N,1);				// extract m minus n only first
